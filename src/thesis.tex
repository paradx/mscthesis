...
The different types of \glspl{UHI} (see \cref{sec:uhi})that can be measured. 
  ...
  %\todo[inline,size=\tiny]{this sentence }
 ...
 \subsection{Methodology}\label{sec:methodology}
  ...
  \subsection{Structure of the Thesis}\label{ssec:structure} 
    ...
    \todo[inline]{Check all sections are covered}
%
\newpage
\section{Background}\label{sec:background}
  \subsection{Urban Heat Islands}\label{sec:uhi}
    ...
    \todo[inline]{oke2002boundary add radiation equeation}
...
%
    \subsubsection{Atmospheric Urban Heat Islands}\label{sec:at_uhi}
      ...
      High surface sealing and lack of vegetation reduce surface water availability and diminish evaporation and the cooling effect of latent heat causing more thermal energy to be available as sensible heat. \todo[inline,size=\tiny]{source (sailor?  or first source)}
...
%
    ...
    \subsubsection{Mitigation techniques for Urban Heat Islands}\label{ssec:mitigation}
      ...
      While high relative humidity at high temperatures has significant health risks (see \cref{sec:heatIndex}) increases in air humidity reduce temperature. %, the amount of temperature reduced by latent heat is larger then the i 
      \cite{Olga1993}
      \todo[inline]{finish and source}
      ...
      \todo[inline]{add formular and image} 
      % 
...
\section{Image Processing}\label{sec:imgProcessing}
    ...
    \todo[inline,size=\tiny]{Might want to add examples here?}
    ...
    \todo[inline,size=\tiny]{external source }
...
%
  ...
  \subsection{Gabor Feature Detection}\label{sec:gabor}
    ...
    \todo[inline,size=\tiny]{add more accurate description of parameters}
    \noindent
...
\section{Comparable Definition of Urban Heat Islands}\label{sec:definition}
    ...
    \subsection{Definition of Urban Heat Islands}\label{ssec:definition}
      ...
      \parbox{\linewidth}{%
        ...
        corrected for larger settlements.
       \vspace{0.2cm}
       }
      }\\ \\
      %
...
\section{Impact of Land Cover Changes on Urban Heat Islands}\label{sec:LULC}
    ...
    \todo[inline]{Add Essen if we get that far\ldots}
    %
...
\todo[inline]{Add ref to SSIM of skmeans}
...
\todo[inline]{Add calculation of SSIM}
  ...
  \begin{figure}[!htbp]
     ...
     \end{subfigure}
        ...
        \todo[inline]{add peri urban tempreature calcultaion and class composition} 
        ...
        \end{figure}
        \todo[inline]{add transition to UHIs, } 
   ...
   \begin{figure}[!p]
     ...
     \end{subfigure}
        ...
        \todo[inline]{Add violin charts and explain wtf this mean} 
 ...
 \subsection{Conclusions}
  ...
  \section{Impact of Climate Change on Urban Heat Islands}\label{sec:UHITempImp}
    ...
    \subsubsection{Land Surface Temperature Calculation}\label{sec:lstcalc}
      ...
      \end{equation}
      To get the level two data from these \glspl{TOA} measurements the value is corrected for emissivity of the ground using measurements from the \textit{ASTER GED} database (see~\cite{USGSWebsite}).\\
      ...
      To convert the image file containing 16 bit unsigned integers into a temperature \cref{equ:tiftolst} is used, where DN is the integer value. The scaling and offset factors are specific to the product and where taken from~\cite{EROASC2013}.
      % 
...
\begin{landscape}
      \begin{figure}[!p]
      %  \begin{subfigure}[b]{\textwidth}
          \includesvg[width=\linewidth]{img/hotdaysbremen_s1940}
...
\begin{landscape}
      \begin{figure}[!p]
           \includesvg[width=\linewidth]{img/hotdaysbremen_s1990}
...
\end{landscape}
   ...
   \end{figure}
      \Cref{fig:uhis2019} shows two summer days in Bremen with different temperatures and the influence of temperature onto the \glspl{UHI} size and shapes. 
      ...
      %This also causes the maximum temperature of the individual \glspl{UHI} to rise (see  % \subsubsection{Essen}
