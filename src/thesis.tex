...
\section{Introduction}\label{sec:intro}
  ...
  These are expected to appear with intensities that where nearly impossible without the impact of climate change \autocite{Bador2016}.
...
%
 ...
 \subsection{Methodology}\label{sec:methodology}
  ...
  \subsubsection{Procedure}\label{ssec:procedure} 
    ...
    \Cref{sec:pipeline} goes into the processing steps in more detail. \todo[inline]{check or change?}
...
\section{Background}\label{sec:background}
  ...
  \subsection{Air Temperature}\label{ssec:airtemperature}
    ...
    \todo[inline]{Add c1-c9? Fix Reference}
  ...
  \subsection{Wet Bulb Globe Temperature}
    ...
    The simpler \gls{HI} is a way of calculating this impact while not taking radiant heat into account, the WBGT does incorporate this effect into this calculation.
...
\section{Comparable Definition of Urban Heat Islands}\label{sec:definition}
    ...
    \subsection{Definition of Urban Heat Islands}\label{ssec:definition}
      ...
      \parbox{\linewidth}{%
        ...
        where the temperature is at least 3 standard deviations above the average temperature of the urban adjacent rural buffer zone, corrected for larger settlements. 
       }
      }\\ \\
      %
...
\section{Impact of Land Cover Changes on Urban Heat Islands}\label{sec:LULC}
    ...
    \end{figure}
    \noindent
    The filter bank with these kernels act as a edge and structure detection algorithm shown in \cref{fig:gaborresults}, that shows extracted features of some example bands.\\
    ...
    This step was crucial in extracting and emphasizing spatial patterns and textures, thereby facilitating more distinct classification of land cover types.
      For classifying surfaces the k-means unsupervised machine learning algorithm was used, the number of classes was varied between 8 and 12 classes, giving best results in terms of minimal \gls{MSE} and highest \gls{SSIM} when using 11 classes.
      The quality of the feature vectors was judged by the output of the classification that is covered below. 
      The feature enhanced data of images of Bremen were used of multiple years to create the unsupervised k-means clustering model.\todo[inline,size=\tiny]{add more}
      ...
      During the available time neither classification nor reference data showed significant land use changes within the urban area of Bremen. The majority of changes where at the interfaces between two classes that are caused by errors in georeferencing, naturally occurring changes in vegetation growth and uncertainties due to spacial resolution.
    ...
    \subsection{Validation of Classification results}\label{sec:references}
      ...
      Since the reference data only uses 8 classes, the used classes where mapped to the best fitting class (as indicated in \cref{tab:land_cover_classes}). 
      ...
      The Esri 10m-Land Use Dataset was obtained using the ESRI Land cover explorer~\autocite{Zhang} that was scaled to the 30 m resolution of the Landsat images.\\
...
%
   ...
   \end{figure}
      ...
      To calculate the diff between \gls{LULC} at different points in time, a median filter is applied to reduce single pixel changes at class interfaces.
       \todo[inline]{add violin with class differences } 
    ...
    No \glspl{UHI} where dominated by any class associated with high vegetation. 
...
\end{landscape}
    ...
    \Glspl{UHI} smaller then 5 pixel (4500 m$^2$) where excluded, to mitigate impact of outliers. 
...
\section{Conclusion}\label{sec:conclusion}
    ...
    The use of indices like the \gls{HI} (see \cref{sec:heatIndex}) are useful tools 
\subsection{Summary}
...
\section{Appendix}
  ...
  \subsection{Additional Figures}
    \begin{figure}[!htbp]
        \includegraphics[width=\textwidth]{img/UrbanAreaStuhr.png}
